...
function selectLensByName(name) {
  ...
  var firstItem = pool.shift();
  setSelectedLens(firstItem);
}
...
function calculate() {
  ...
  var row0label = 'Price Per Box';
  var row0ours  = fmt(retailPerBox);
  var row0comp  = fmt(compPerBox);

  var row1label = 'Boxes (per eye)';
  var row1ours  = String(boxes);
  var row1comp  = String(boxes);

  var row2label = 'Eyes';
  var row2ours  = String(eyes);
  var row2comp  = String(eyes);

  var row3label = 'Gross Total';
  var row3ours  = fmt(ourGross);
  var row3comp  = fmt(compGross);

  var row4label = 'Rebate Applied';
  var row4ours  = rebate     > 0 ? '-' + fmt(rebate)     : '-';
  var row4comp  = compRebate > 0 ? '-' + fmt(compRebate) : '-';

  var row5label = '<strong>Net Total</strong>';
  var row5ours  = '<strong>' + fmt(ourNet)  + '</strong>';
  var row5comp  = '<strong>' + fmt(compNet) + '</strong>';

  var tbodyHtml = '';

  tbodyHtml += '<tr><td>' + row0label + '</td><td class="col-ours">' + row0ours + '</td><td class="col-comp">' + row0comp + '</td></tr>';
  tbodyHtml += '<tr><td>' + row1label + '</td><td class="col-ours">' + row1ours + '</td><td class="col-comp">' + row1comp + '</td></tr>';
  tbodyHtml += '<tr><td>' + row2label + '</td><td class="col-ours">' + row2ours + '</td><td class="col-comp">' + row2comp + '</td></tr>';
  tbodyHtml += '<tr><td>' + row3label + '</td><td class="col-ours">' + row3ours + '</td><td class="col-comp">' + row3comp + '</td></tr>';
  tbodyHtml += '<tr><td>' + row4label + '</td><td class="col-ours">' + row4ours + '</td><td class="col-comp">' + row4comp + '</td></tr>';
  tbodyHtml += '<tr><td>' + row5label + '</td><td class="col-ours">' + row5ours + '</td><td class="col-comp">' + row5comp + '</td></tr>';
